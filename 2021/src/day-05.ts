...
const normalize = ([ x1, y1, x2, y2 ]: Line): [ boolean, number, number, number, number ] => {
...
const plotStraightLine = ([ x1, y1, x2, y2 ]: Line, pixels: Array<number>, w: number) => {
  for (let x = x1; x <= x2; x++) {
    for (let y = y1; y <= y2; y++) {
      plot(x, y, pixels, w)
    }
...
export const plotOceanFloor = (lines: Line[], width: number = 1000, height: number = 1000): number[] => {
...
export const countOverlappingPoints = (pixels: number[]): number =>
  pixels.reduce((count, v) => v >= 2
                              ? count + 1
                              : count, 0)


export const drawDiagram = (pixels: Array<number>, w: number, h: number): string => {
  const output: string[] = []

  for (let y = 0; y < h; y++) {
    output.push(pixels.slice(y * w, y * w + w)
                      .map(p => p == 0 ? `.` : p)
                      .join(``))
  }
